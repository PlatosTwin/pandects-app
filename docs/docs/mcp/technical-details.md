...
- `get_agreement_trends` accepts a `sections` selector (`ownership`, `target_industries`, `pairings`, `naics_catalog`) and defaults to `[ownership, target_industries]`; the large `pairings` (every industry-by-industry cell) and `naics_catalog` (full NAICS hierarchy) sections are opt-in, and the response echoes `sections_returned`. It also accepts `year_min`/`year_max` to slice the per-year arrays (ownership mix/deal size, target-industry distributions) to a window instead of returning every year; the buyer-type matrix aggregates across all years and is unaffected, and the applied window is echoed under `year_filter`
