...
- `get_agreements_summary` reports the corpus as the retrieval tools can actually see it. All three figures describe one universe — the agreements this server can return — so ratios between them are safe. `agreements` is counted over the same expressions every listing builds (the join to the latest verified XML row, plus the public-eligibility gate), so it equals an unfiltered `search_agreements` `total_count`. `pages` counts only those agreements' pages. `sections` is read from the `summary_data` rollup unchanged, because the ETL builds that figure through the same XML join and it is already retrievable-scoped by construction. What the rollup does **not** apply is that XML join for its agreement and page counts: `summary_data` already applies the public-eligibility gate, so the single missing predicate is the XML one. On the current corpus the rollup counts 10 784 agreements against 9 902 retrievable — 882 agreements (8.2% of the rollup figure) that no tool here can return, and 31 830 pages belonging to them
...
- `search_sections` returns a top-level `standard_id_labels` map resolving every `standard_id` present in the page to its taxonomy label, so a client does not need a follow-up `get_clause_taxonomy` call just to name the nodes it matched
