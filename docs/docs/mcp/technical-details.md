...
- `get_agreements_summary` reports the corpus as the retrieval tools can actually see it, and each figure equals what the corresponding tool reports, so it is a sound basis for coverage math rather than the inflated ingestion totals. `agreements` is counted over the same expressions every listing builds (the join to the latest verified XML row, plus the public-eligibility gate), so it equals an unfiltered `search_agreements` `total_count`; `pages` counts the pages of exactly those agreements, so `pages / agreements` is a safe ratio. `sections` counts the same search index `search_sections` paginates, so it equals an unfiltered `search_sections` total. One caveat on cross-figure ratios: that section index is built from the XML-latest join **without** the public-eligibility gate the agreement and page counts apply, so `sections / agreements` is only exact while no gated-unverified agreement has a latest verified XML row (true today — the count is zero — because gating is resolved before an agreement reaches that state, but it is a property of the pipeline, not of these two queries). None of the three is read from the `summary_data` rollup any more: that rollup applies the eligibility gate but not the XML join, so its agreement and page counts include records no tool here can return, and its section count is built through a *stricter* XML join than the backend's (the ETL requires `status = 'verified'`; the backend also accepts a NULL status), which agrees with the backend only while no latest row has a NULL status. On the current corpus the rollup counts 10 784 agreements against 9 902 retrievable — 882 agreements (8.2% of the rollup figure) that no tool here can return, and 31 830 pages belonging to them
