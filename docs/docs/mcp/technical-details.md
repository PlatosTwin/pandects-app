...
- `search_sections` is a clause-language retrieval surface, not a normalized document-facts surface. Its `text_query` parameter performs case-insensitive lexical full-text retrieval across current section text and composes with clause-family `standard_id` plus every structured M&A filter. `text_match_mode` controls whether the query is matched as one ordered-token `phrase` (the default), `all_terms`, or `any_terms`; append `*` only to the end of a word for prefix matching (for example, `sandbag*`). Phrase mode verifies token order against normalized text rather than delegating to MariaDB's much slower quoted-phrase evaluator; this preserves short contextual language such as `no shop` and makes multi-word phrase-prefix searches work. Queries are capped at 256 characters and 24 parsed terms, with at most four prefix terms and at least three characters before each `*`. Phrase queries need at least one indexed word or prefix; `all_terms` and `any_terms` reject shorter terms and default InnoDB stopwords because those words cannot be matched independently. Phrase mode can verify them in context when another indexed word supplies the candidate set. Text is normalized with Unicode NFKC, case-folding, and whitespace collapse. Boolean punctuation and operators are tokenized as separators rather than interpreted or passed through to the database; blank or punctuation-only queries, leading wildcards, and infix wildcards are rejected. Each text-search statement runs under a 20-second database bound. Phrase mode has two query plans with opposite failure modes: the FULLTEXT plan narrows by the candidate index first, so its cost scales with how many sections contain the phrase's words and it suits rare phrases; the date-ordered plan walks the sort index applying the token-order check per row, so it stops early exactly when the phrase is common enough to make the candidate set unusable. The first request for a filter signature runs one plan under a short budget and falls back to the other, then remembers the winner for later pages of the same search. A phrase therefore returns a validation error only when both plans exceed the bound, and that error asks for a longer or more distinctive phrase — extra words shrink the candidate set, whereas metadata filters do not help, because the full-text scan runs ahead of them. If the text index is not present on the server, `text_query` returns a validation error saying so instead of empty results. Search calls are also rate-limited per account — 120 `search_sections`/`search_agreements`/`search_tax_clauses` calls per minute in total, and 30 `search_sections` calls carrying a `text_query` per minute — and a call over either limit returns an agent-visible `-32602` saying how many seconds to wait before retrying. This is lexical, not semantic, search: use `suggest_clause_families` (or `get_clause_taxonomy`) to map a legal concept to taxonomy nodes, and combine the resulting `standard_id` with `text_query` when both conceptual coverage and exact drafting language matter
- `search_sections` exposes `count_mode` and returns `count_metadata` plus `interpretation` so clients can tell when totals are exact versus estimated and when taxonomy is acting as a proxy. `total_count` counts matching sections. In automatic phrase mode it is a stable upper bound obtained by exactly counting the indexed all-term candidate set; request `count_mode=exact` to apply the ordered-token postfilter to the count and also receive the corpus-wide `total_agreement_count`. A phrase common enough that even the candidate count exceeds its budget reports `total_count` as a lower bound — the rows seen so far, plus one when a further page exists — with `method: filtered_lower_bound` and `planning_reliability: low`; read it together with `has_next` rather than as a total, and note that `count_mode=exact` degrades the same way rather than failing the search. `page_unique_agreement_count` only counts distinct agreements on the returned page
- Text search is resource-bounded independently of ordinary section listing: `any_terms` accepts at most eight alternatives, and text searches are limited to page 100. Narrow broad searches with deal or taxonomy filters instead of forcing a very large FULLTEXT offset
...
- `search_sections` accepts `include_snippet: true` (with optional `snippet_focus_terms` and `snippet_max_chars`, default 400, range 120–1200) to return a plain-text `snippet` plus `matched_terms`, `source_length`, and `monetary_values` on every result. When a text query is present and `snippet_focus_terms` is omitted, the handler derives focus terms from the query automatically (and removes a trailing prefix `*`). This collapses the usual `search_sections` → `get_section_snippets_batch` pair into one call for the common "find the clause and read what it says" workflow. XML hydration is skipped for the default metadata-only MCP result and enabled only when a snippet or XML was requested. Snippets stay off by default and are far smaller than `include_xml`
...
- For literal searches over section text, use `search_sections`; it applies the full-text index before pagination and can return compact matching snippets. For whole-document analysis that cannot be expressed as a section query, filter with `list_agreements` using `fields: ["agreement_uuid", "url", ...]` and fetch full agreement bodies from the returned EDGAR `url` out-of-band. Do not pull full agreement texts through MCP responses at scale: a full-row `list_agreements` page at `page_size=100` already runs roughly 100k characters before any XML is attached, and `get_agreement`/`get_sections_batch` XML is larger still
