"""Module-level helpers for the MCP routes blueprint.

JSON-RPC envelopes, SSE streaming, prompt/resource definitions, request
parsing, and structured logging. Pulled out of ``__init__`` so the main
file can focus on ``register_mcp_routes``.
"""
...
_PROMPT_TEMPLATES: tuple[dict[str, object], ...] = (
    {
        ...
        "template": (
            ...
            "   with those standard_ids, agreement_uuid, and include_snippet=true. When the focus includes\n"
            "   literal drafting language, also pass it as text_query to narrow the results and focus snippets\n"
    ...
    },
    {
        ...
        "template": (
            ...
            "3. Call `search_sections` with the taxonomy match plus filters. Add text_query when the concept\n"
            "   includes literal drafting language, and pass include_snippet=true so the matching text arrives\n"
            "   with each result. Use count_mode=exact when reporting section or agreement prevalence.\n"
