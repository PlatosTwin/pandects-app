...
from etl.defs.f_xml_asset import _apply_xml_verify_batch_output, xml_verify_asset
...
class XMLVerifyAssetTests(unittest.TestCase):
    ...
    def test_xml_verify_asset_hard_invalid_sets_status_source_to_asset(self) -> None:
        ...
        with (
            ...
            patch("etl.defs.f_xml_asset.apply_xml_gating", return_value=0),
            patch("etl.defs.f_xml_asset.apply_gating", return_value=SimpleNamespace()),
            patch("etl.defs.f_xml_asset.refresh_summary_data", return_value=None),
        ):
        ...
        hard_invalid_update_sql = next(
            sql for sql, _ in conn.executed if "SET status = 'invalid'" in sql
        )
