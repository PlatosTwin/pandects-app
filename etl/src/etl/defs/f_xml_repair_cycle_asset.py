...
def regular_ingest_post_repair_build_xml_asset(
    ...
    target_agreement_uuids = load_active_scope_for_job(
        ...
        fallback_agreement_uuids=reconciled_agreement_uuids,
    )
    target_agreement_uuids = sorted(set(target_agreement_uuids))
    scoped_limit = max(agreement_batch_size, len(target_agreement_uuids))
    if not target_agreement_uuids:
        context.log.info("regular_ingest_post_repair_build_xml_asset: no upstream agreements from reconcile_tags.")
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    with engine.begin() as conn:
        ...
        for failure in xml_generation_failures:
            context.log.warning(
                "regular_ingest_post_repair_build_xml_asset: skipping XML generation due to parse error for agreement_uuid=%s: %s",
                failure.agreement_uuid,
                failure.error,
            )
        if not xml:
        ...
        _ = refresh_latest_sections_search(conn, db.database, generated_agreement_uuids)

    run_post_asset_refresh(context, db, pipeline_config)
    mark_logical_run_stage_completed(
        db=db,
        job_name="regular_ingest",
        stage_name="regular_ingest_post_repair_build_xml",
    )
    return sorted(set(generated_agreement_uuids))
...
def regular_ingest_post_repair_verify_xml_asset(
    ...
    target_agreement_uuids = load_active_scope_for_job(
        ...
        db=db,
        job_name="regular_ingest",
        fallback_agreement_uuids=rebuilt_agreement_uuids,
    )
    target_agreement_uuids = sorted(set(target_agreement_uuids))
    scoped_limit = max(agreement_batch_size, len(target_agreement_uuids))
    ...
    if not target_agreement_uuids:
        context.log.info("regular_ingest_post_repair_verify_xml_asset: no upstream agreements from regular_ingest_post_repair_build_xml_asset.")
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    if not eligible_uuids:
        context.log.info(
            "regular_ingest_post_repair_verify_xml_asset: no upstream-selected latest ai_repair_attempted XML rows with status IS NULL."
        )
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    if not lines:
        run_post_asset_refresh(context, db, pipeline_config)
        with engine.begin() as conn:
            return _fetch_latest_verified_agreement_uuids(
                conn,
                xml_table=xml_table,
                agreement_uuids=selected_for_verify,
            )
    ...
    if resume_openai_batches:
        ...
        if existing_batch is not None:
            _ = _resume_xml_verify_batch(
                ...
                batch_row=existing_batch,
                agreement_uuids=selected_for_verify,
                log_prefix="regular_ingest_post_repair_verify_xml_asset",
                hard_invalid_updated=hard_invalid_updated,
            )
            with engine.begin() as conn:
                return _fetch_latest_verified_agreement_uuids(
    ...
    for line in lines:
        _ = jsonl_buf.write(json.dumps(line, ensure_ascii=False) + "\n")
    jsonl_bytes = io.BytesIO(jsonl_buf.getvalue().encode("utf-8"))
    jsonl_bytes.name = "regular_ingest_post_repair_xml_verify_requests.jsonl"
    input_file = client.files.create(purpose="batch", file=jsonl_bytes)
    completion_window = "24h"
    batch = client.batches.create(
        input_file_id=input_file.id,
...
@dg.asset(
    name="05-09_ingestion_cleanup_a_post_repair_build_xml",
    ins={"reconciled_agreement_uuids": dg.AssetIn(key=ingestion_cleanup_a_reconcile_tags.key)},
)
def ingestion_cleanup_a_post_repair_build_xml_asset(
    context: AssetExecutionContext,
    db: DBResource,
    pipeline_config: PipelineConfig,
    reconciled_agreement_uuids: List[str],
    ...
    scoped_limit = max(agreement_batch_size, len(target_agreement_uuids))
    if not target_agreement_uuids:
        context.log.info("ingestion_cleanup_a_post_repair_build_xml_asset: no upstream agreements from ingestion_cleanup_a_reconcile_tags.")
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    with engine.begin() as conn:
        ...
        for failure in xml_generation_failures:
            context.log.warning(
                "ingestion_cleanup_a_post_repair_build_xml_asset: skipping XML generation due to parse error for agreement_uuid=%s: %s",
                failure.agreement_uuid,
                failure.error,
            )
        if not xml:
    ...
    mark_logical_run_stage_completed(
        db=db,
        job_name="ingestion_cleanup_a",
        stage_name="ingestion_cleanup_a_post_repair_build_xml",
    )
    return sorted(set(generated_agreement_uuids))


@dg.asset(
    name="05-10_ingestion_cleanup_a_post_repair_verify_xml",
    ins={"rebuilt_agreement_uuids": dg.AssetIn(key=ingestion_cleanup_a_post_repair_build_xml_asset.key)},
)
def ingestion_cleanup_a_post_repair_verify_xml_asset(
    context: AssetExecutionContext,
    db: DBResource,
    pipeline_config: PipelineConfig,
    rebuilt_agreement_uuids: List[str],
    ...
    if not target_agreement_uuids:
        context.log.info("ingestion_cleanup_a_post_repair_verify_xml_asset: no upstream agreements from ingestion_cleanup_a_post_repair_build_xml_asset.")
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    if not eligible_uuids:
        context.log.info(
            "ingestion_cleanup_a_post_repair_verify_xml_asset: no upstream-selected latest ai_repair_attempted XML rows with status IS NULL."
        )
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    llm_targets = sorted({_parse_custom_id(str(line["custom_id"])) for line in lines})
    if not llm_targets:
        raise ValueError("ingestion_cleanup_a_post_repair_verify_xml_asset: no (agreement_uuid, version) targets derived from LLM lines.")
    active_run = load_active_logical_run(db=db, job_name="ingestion_cleanup_a")
    verify_batch_key = build_logical_batch_key(
        logical_run_id=None if active_run is None else str(active_run["logical_run_id"]),
        stage_name="ingestion_cleanup_a_post_repair_verify_xml",
        default_key=agreement_version_batch_key(llm_targets),
    )
    ...
    if resume_openai_batches:
        ...
        if existing_batch is not None:
            _ = _resume_xml_verify_batch(
                ...
                batch_row=existing_batch,
                agreement_uuids=selected_for_verify,
                log_prefix="ingestion_cleanup_a_post_repair_verify_xml_asset",
                hard_invalid_updated=hard_invalid_updated,
            )
            with engine.begin() as conn:
                return _fetch_latest_verified_agreement_uuids(
    ...
    for line in lines:
        _ = jsonl_buf.write(json.dumps(line, ensure_ascii=False) + "\n")
    jsonl_bytes = io.BytesIO(jsonl_buf.getvalue().encode("utf-8"))
    jsonl_bytes.name = "ingestion_cleanup_a_post_repair_verify_requests.jsonl"
    input_file = client.files.create(purpose="batch", file=jsonl_bytes)
    completion_window = "24h"
    batch = client.batches.create(
        input_file_id=input_file.id,
...
@dg.asset(
    name="05-13_ingestion_cleanup_b_post_repair_build_xml",
    ins={"reconciled_agreement_uuids": dg.AssetIn(key=ingestion_cleanup_b_reconcile_tags.key)},
)
def ingestion_cleanup_b_post_repair_build_xml_asset(
    context: AssetExecutionContext,
    db: DBResource,
    pipeline_config: PipelineConfig,
    reconciled_agreement_uuids: List[str],
    ...
    scoped_limit = max(agreement_batch_size, len(target_agreement_uuids))
    if not target_agreement_uuids:
        context.log.info("ingestion_cleanup_b_post_repair_build_xml_asset: no upstream agreements from ingestion_cleanup_b_reconcile_tags.")
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    with engine.begin() as conn:
        ...
        for failure in xml_generation_failures:
            context.log.warning(
                "ingestion_cleanup_b_post_repair_build_xml_asset: skipping XML generation due to parse error for agreement_uuid=%s: %s",
                failure.agreement_uuid,
                failure.error,
            )
        if not xml:
...
@dg.asset(
    name="05-14_ingestion_cleanup_b_post_repair_verify_xml",
    ins={"rebuilt_agreement_uuids": dg.AssetIn(key=ingestion_cleanup_b_post_repair_build_xml_asset.key)},
)
def ingestion_cleanup_b_post_repair_verify_xml_asset(
    context: AssetExecutionContext,
    db: DBResource,
    pipeline_config: PipelineConfig,
    rebuilt_agreement_uuids: List[str],
    ...
    if not target_agreement_uuids:
        context.log.info("ingestion_cleanup_b_post_repair_verify_xml_asset: no upstream agreements from ingestion_cleanup_b_post_repair_build_xml_asset.")
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    if not eligible_uuids:
        context.log.info(
            "ingestion_cleanup_b_post_repair_verify_xml_asset: no upstream-selected latest ai_repair_attempted XML rows with status IS NULL."
        )
        run_post_asset_refresh(context, db, pipeline_config)
        return []
    ...
    llm_targets = sorted({_parse_custom_id(str(line["custom_id"])) for line in lines})
    if not llm_targets:
        raise ValueError("ingestion_cleanup_b_post_repair_verify_xml_asset: no (agreement_uuid, version) targets derived from LLM lines.")
    ...
    if resume_openai_batches:
        ...
        if existing_batch is not None:
            _ = _resume_xml_verify_batch(
                ...
                batch_row=existing_batch,
                agreement_uuids=selected_for_verify,
                log_prefix="ingestion_cleanup_b_post_repair_verify_xml_asset",
                hard_invalid_updated=hard_invalid_updated,
            )
            with engine.begin() as conn:
                return _fetch_latest_verified_agreement_uuids(
    ...
    for line in lines:
        _ = jsonl_buf.write(json.dumps(line, ensure_ascii=False) + "\n")
    jsonl_bytes = io.BytesIO(jsonl_buf.getvalue().encode("utf-8"))
    jsonl_bytes.name = "ingestion_cleanup_b_post_repair_verify_requests.jsonl"
    input_file = client.files.create(purpose="batch", file=jsonl_bytes)
    completion_window = "24h"
    batch = client.batches.create(
        input_file_id=input_file.id,
