...
describe("SearchActionsBar", () => {
  ...
  it("lets the mobile download action grow with the toolbar row", () => {
    const { container } = renderBar({ selectedSize: 2 });

    const wrapper = container.querySelector('[aria-label="Download CSV"]')?.parentElement;
    expect(wrapper?.className).toContain("flex-1");
    expect(wrapper?.className).toContain("sm:flex-none");
  });
